...
const EyeIcon = ({ onClick }: { onClick: () => void }) => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-gray-400 cursor-pointer hover:text-white" onClick={onClick}>
...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // For OAuth the browser will redirect; no further handling here
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        // Jika session tidak ada, biasanya ini berarti: user dibuat tapi belum dikonfirmasi (email confirmation)
        const session = (result.data as any)?.session;
        if (!session) {
          setError("Akun belum aktif / perlu konfirmasi email. Silakan cek inbox (atau spam).");
        } else {
          // berhasil login
          router.push("/");
          router.refresh();
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gray-900">
      <div className="relative z-10 hidden lg:flex w-1/2 p-12 items-center justify-center">
        <div className="w-full max-w-md">
          <Link href="/">
            <Image src="/images/logo/logo_brown_batiklens.svg" alt="BatikLens Logo" width={400} height={200} className="object-contain" />
          </Link>
        </div>
      </div>
