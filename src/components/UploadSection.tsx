...
// --- Komponen & Tipe Data (TIDAK DIUBAH) ---
const UploadIcon = () => (
...
const UploadSection = () => {
    // ==========================================================
    // SEMUA LOGIKA AI & BACKEND DI BAWAH INI TIDAK DIUBAH SAMA SEKALI
    // ==========================================================
    ...
    const fetchBatikDetails = async (motifName: string): Promise<Batik | null> => {
        try {
            const { data, error } = await supabase.from('motifs').select('name, origin, philosophy').eq('name', motifName).single();
            if (error) {
    ...
    const handleDetectClick = async () => {
        ...
        try {
            ...
            const tensor = tf.browser.fromPixels(imageElement).resizeBilinear([224, 224]).toFloat().expandDims(0).div(tf.scalar(255));
            ...
            if (details) {
                ...
                setAnalysisResult({ name: predictedBatikName, origin: "Data tidak ditemukan", philosophy: "Informasi detail untuk motif ini belum tersedia di database kami." });
            }
        } catch (error) {
            console.error("Error saat deteksi:", error);
            setAnalysisResult({ name: "Gagal Deteksi", origin: "-", philosophy: "Terjadi kesalahan saat proses identifikasi. Mohon coba gambar lain." });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const onDrop = (e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); handleFileChange(e.dataTransfer.files[0]); };
    const onDragOver = (e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); };
    const onFileInputChange = (e: React.ChangeEvent<HTMLInputElement>) => { handleFileChange(e.target.files?.[0] || null); };
    // ==========================================================
    // AKHIR DARI BAGIAN LOGIKA BACKEND
    // ==========================================================
    
    // [BAGIAN TAMPILAN (JSX) YANG SUDAH DIDESAIN ULANG]
    return (
            ...
            <div className="relative z-10 container mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8">
                <div className="text-center max-w-3xl mx-auto mb-12">
                    ...
                    <p className="mt-4 text-gray-300 leading-relaxed">
                        Unggah gambar kain batik Anda, dan biarkan teknologi AI kami mengidentifikasi nama, asal, dan filosofi di baliknya dalam hitungan detik.
                    </p>
                </div>

                <div className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-3xl shadow-xl grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                    ...
                    <div className="flex flex-col p-4 h-full">
                        ...
                        <div className="mt-4 space-y-2 text-gray-600 dark:text-gray-300 text-base flex-grow">
                            <p><span className="font-semibold text-black dark:text-white">Asal:</span> {analysisResult ? analysisResult.origin : statusMessage}</p>
                            <div className="max-h-[200px] overflow-y-auto pr-2 space-y-1">
                                <span className="font-semibold text-black dark:text-white">Filosofi:</span>
